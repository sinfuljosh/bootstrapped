...
<h1>{$movie.title|ss} {if $movie.year != ''}({$movie.year}){/if}{if $movie.cover == 1}<img src="{$smarty.const.WWW_TOP}/covers/movies/{$movie.imdbID}-cover.jpg" class="cover" alt="{$movie.title|ss}" align="right" />{/if}</h1>
<h2>
